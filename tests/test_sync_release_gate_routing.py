...
Aligned to project-manager-agent's actual implementation (bin/release_gate.py,
PR#11): the release-gate marker is NOT a separate freetext field, it's the
*existing* `**Status:**` line. Phase 1 (capture) writes
`**Status:** wartet-auf-freigabe`; Phase 2 (release, only on Leo's explicit
"schick los"/"mach") stamps `**Status:** freigegeben (<ISO timestamp>)`.

Covers:
  1. `**Status:** wartet-auf-freigabe` ticket -> routes to the fixed
     "wartet auf Freigabe" section (even though that status value has no
     entry in `status_to_column`).
  2. `wartet-auf-freigabe` + `Provenance: leo-direct` -> release gate still
     wins (checked before provenance).
  3. ticket with a normal status (no pending marker) -> falls through to
     normal routing (provenance routing still applies on MANAGER_BOARD, same
     as T-297) — the gate only fires on the exact pending value, so every
     ticket predating the marker is unaffected by it.
  4. `wartet-auf-freigabe` flips to `freigegeben (<ts>)` -> ticket moves OUT
     of "wartet auf Freigabe" back to its normal target on the next sync
  ...
  5. a non-MANAGER_BOARD config (no `release_pending_section` set) ignores
     the `wartet-auf-freigabe` Status value entirely — proves the feature is
     opt-in / scoped.
...
TICKETS = {
    # 1. captured, not yet released (`**Status:** wartet-auf-freigabe`) ->
    # "wartet auf Freigabe", even though that status value has no
    # `status_to_column` entry of its own (falls to default_column absent
    # the gate).
    ...
    # 3. normal status, no pending marker -> normal routing (falls through to
    # T-297 provenance routing since Provenance is missing on this ticket).
    "MB-3_no_release_marker.md": (
        "# MB-3 — No release marker\n\n"
        "**Status:** testing\n\n"
        "Legacy ticket predating the release-gate convention.\n"
    ),
}
...
def main() -> int:
    ...
    print("OK: Status wartet-auf-freigabe -> 'wartet auf Freigabe' (no status_to_column entry needed)")

    assert by_id["MB-2"] == "wartet auf Freigabe", f"pending+leo-direct misrouted: {by_id}"
    print("OK: Status wartet-auf-freigabe beats Provenance: leo-direct (gate checked first)")

    # MB-3 has neither the pending marker nor Provenance -> falls through the
    # release gate (opt-in, no marker match) into T-297's provenance routing
    # (also opt-in, no marker), which sends it to "maintainance" — proves the
    # two axes compose rather than fight each other.
    assert by_id["MB-3"] == "maintainance", f"no-marker ticket misrouted: {by_id}"
    print("OK: normal status -> falls through to provenance routing (composes with T-297)")
    ...
    # 4. release flip: rewrite MB-1's Status line to `freigegeben (<ts>)`
    # (project-manager-agent's actual Phase-2 stamp, keep it leo-direct so
    # the expected fallback is unambiguous), re-sync -> moves OUT of the gate
    # section back to its (now provenance-routed) target. "freigegeben" has
    # no `status_to_column` entry either, so the fallback target is
    # MANAGER_BOARD's default_column ("backlog").
    (tdir / "MB-1_captured_pending.md").write_text(
        "# MB-1 — Captured, pending release\n\n"
        "**Status:** freigegeben (2026-07-10T12:00:00)\n"
        "Provenance: leo-direct\n\n"
    ...
    assert col_change == "backlog", f"release flip didn't move MB-1 out: {col_change}"
    print("OK: Status flips to freigegeben (<ts>) -> ticket leaves 'wartet auf Freigabe' on next sync")
    ...
    assert col2 == "backlog", f"non-opted-in board routing changed: {col2}"
    print("OK: board without release_pending_section (e.g. CORTEX_BOARD) ignores wartet-auf-freigabe Status")
